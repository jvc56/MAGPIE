...
void write_letter_minimum(InferenceRecord * record, Rack * rack, Rack * bag_as_rack, char * inference_string, uint8_t letter, int minimum, int number_of_actual_tiles_played) {
    ...
    double random_probability = get_probability_for_random_minimum_draw(bag_as_rack, rack, letter, minimum, number_of_actual_tiles_played);
...
void write_letter_line(Game * game, InferenceRecord * record, Rack * rack, Rack * bag_as_rack, Stat * letter_stat, char * inference_string, uint8_t letter, int max_duplicate_letter_draw, int number_of_actual_tiles_played) {
    ...
    for (int i = 1; i <= max_duplicate_letter_draw; i++) {
        write_letter_minimum(record, rack, bag_as_rack, inference_string, letter, i, number_of_actual_tiles_played);
    }
    write_string_to_end_of_buffer(inference_string, "\n");
}

void write_inference_record(char * buffer, InferenceRecord * record, Game * game, Rack * rack, Rack * bag_as_rack, Stat * letter_stat, Rack * actual_tiles_played) {
    ...
    if (total_draws > 0) {
        for (int i = 0; i < (int)game->gen->letter_distribution->size; i++) {
            write_letter_line(game, record, rack, bag_as_rack, letter_stat, buffer, i, max_duplicate_letter_draw, actual_tiles_played->number_of_letters);
        }
    }
...
void print_inference(Inference * inference, Rack * actual_tiles_played) {
    ...
    if (!is_exchange) {
        ...
        write_int_to_end_of_buffer(inference_string, inference->number_of_tiles_exchanged);
    }
    ...
    write_inference_record(records_string, inference->leave_record, game, inference->leave, inference->bag_as_rack, letter_stat, actual_tiles_played);
    if (is_exchange) {
	    sprintf(records_string + strlen(records_string), "\n\nTiles Exchanged\n\n");
        write_inference_record(records_string, inference->exchanged_record, game, inference->leave, inference->bag_as_rack, letter_stat, actual_tiles_played);
	    sprintf(records_string + strlen(records_string), "\n\nRack\n\n");
        write_inference_record(records_string, inference->rack_record, game, inference->leave, inference->bag_as_rack, letter_stat, actual_tiles_played);
    }
