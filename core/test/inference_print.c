...
void write_leave_rack(char * buffer, LeaveRack * leave_rack, int index, uint64_t total_draws, LetterDistribution * letter_distribution) {
    ...
    if (leave_rack->exchanged->empty) {
        ...
        write_rack_to_end_of_buffer(exchanged_string, letter_distribution, leave_rack->exchanged);
        sprintf(buffer + strlen(buffer),"%-3d %-7s %-7s %-6.2f %-6d\n", index + 1, leave_string, exchanged_string, ((double)leave_rack->draws/total_draws) * 100, leave_rack->draws);
    }
}

void write_letter_minimum(InferenceRecord * record, Rack * rack, Rack * bag_as_rack, char * inference_string, uint8_t letter, int minimum, int number_of_tiles_played_or_exchanged) {
...
void write_letter_line(Game * game, InferenceRecord * record, Rack * rack, Rack * bag_as_rack, Stat * letter_stat, char * inference_string, uint8_t letter, int max_duplicate_letter_draw, int number_of_tiles_played_or_exchanged) {
    ...
    for (int i = 1; i <= max_duplicate_letter_draw; i++) {
        write_letter_minimum(record, rack, bag_as_rack, inference_string, letter, i, number_of_tiles_played_or_exchanged);
    }
    write_string_to_end_of_buffer(inference_string, "\n");
}

void write_inference_record(char * buffer, InferenceRecord * record, Game * game, Rack * rack, Rack * bag_as_rack, Stat * letter_stat, int number_of_tiles_played_or_exchanged) {
    ...
    if (total_draws > 0) {
        for (int i = 0; i < (int)game->gen->letter_distribution->size; i++) {
            write_letter_line(game, record, rack, bag_as_rack, letter_stat, buffer, i, max_duplicate_letter_draw, number_of_tiles_played_or_exchanged);
        }
    }
}
...
void print_inference(Inference * inference, Rack * actual_tiles_played) {
    ...
    if (!is_exchange) {
        ...
        number_of_tiles_played_or_exchanged = actual_tiles_played->number_of_letters;
    } else {
    ...
    write_inference_record(records_string, inference->leave_record, game, inference->leave, inference->bag_as_rack, letter_stat, number_of_tiles_played_or_exchanged);
