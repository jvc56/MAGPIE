...
void write_player_row_to_end_of_buffer(char *buf,
...
void write_board_row_to_end_of_buffer(char *buf,
                                      LetterDistribution *letter_distribution,
                                      Board *board, int row) {
...
void print_game(Game *game) {
  ...
  write_player_row_to_end_of_buffer(gs, game->gen->letter_distribution,
                                    game->players[0], player_0_on_turn_marker);
  write_string_to_end_of_buffer(gs, "\n   ------------------------------  ");
  write_player_row_to_end_of_buffer(gs, game->gen->letter_distribution,
                                    game->players[1], player_1_on_turn_marker);
  write_string_to_end_of_buffer(gs, "\n");

  for (int i = 0; i < BOARD_DIM; i++) {
    write_board_row_to_end_of_buffer(gs, game->gen->letter_distribution,
                                     game->gen->board, i);
    if (i == 0) {
      write_string_to_end_of_buffer(
          gs, " --Tracking-----------------------------------");
    } else if (i == 1) {
      write_string_to_end_of_buffer(gs, " ");
      write_bag_to_end_of_buffer(gs, game->gen->bag,
      ...
      write_user_visible_move_to_end_of_buffer(
          move_string, game->gen->board, game->gen->move_list->moves[i - 2],
          game->gen->letter_distribution);
      sprintf(gs + strlen(gs), " %-3d %-24s %0.2f", i - 1, move_string,
              game->gen->move_list->moves[i - 2]->equity);
    }
    write_string_to_end_of_buffer(gs, "\n");
  }
