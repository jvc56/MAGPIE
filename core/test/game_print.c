...
void write_player_row_to_end_of_buffer(char * buf, LetterDistribution * letter_distribution, Player * player, char * marker) {
	...
	write_rack_to_end_of_buffer(buf, letter_distribution, player->rack);
...
void write_board_row_to_end_of_buffer(char * buf, LetterDistribution * letter_distribution, Board * board, int row) {
	...
	for (int i = 0; i < BOARD_DIM; i++) {
		...
		if (current_letter == ALPHABET_EMPTY_SQUARE_MARKER) {
			...
			write_user_visible_letter_to_end_of_buffer(buf, letter_distribution, current_letter);
		}
...
void print_game(Game * game) {
	...
	write_player_row_to_end_of_buffer(gs, game->gen->letter_distribution, game->players[0], player_0_on_turn_marker);
	write_string_to_end_of_buffer(gs, "\n   ------------------------------  ");
	write_player_row_to_end_of_buffer(gs, game->gen->letter_distribution, game->players[1], player_1_on_turn_marker);
	write_string_to_end_of_buffer(gs, "\n");

	for (int i = 0; i < BOARD_DIM; i++) {
		write_board_row_to_end_of_buffer(gs, game->gen->letter_distribution, game->gen->board, i);
		if (i == 0) {
			write_string_to_end_of_buffer(gs, " --Tracking-----------------------------------");
		} else if (i == 1) {
			write_string_to_end_of_buffer(gs, " ");
			write_bag_to_end_of_buffer(gs, game->gen->bag, game->gen->letter_distribution);
			write_string_to_end_of_buffer(gs, "  ");
			write_int_to_end_of_buffer(gs, game->gen->bag->last_tile_index + 1);
		} else if (i - 2 < game->gen->move_list->count) {
			char move_string[24] = "";
			write_user_visible_move_to_end_of_buffer(move_string, game->gen->board, game->gen->move_list->moves[i-2], game->gen->letter_distribution);
