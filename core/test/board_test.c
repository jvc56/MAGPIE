...
void test_board_cross_set_for_cross_set_index(Game *game, int cross_set_index) {
  ...
  board_clear_cross_set(board, 0, 0, BOARD_HORIZONTAL_DIRECTION, cross_set_index);
  board_set_cross_set_letter(board_get_cross_set_pointer(board, 0, 0,
  ...
  uint64_t cs =
      board_get_cross_set(board, 0, 0, BOARD_HORIZONTAL_DIRECTION, cross_set_index);
...
void test_board() {
  ...
  assert(!board_get_anchor(board, 3, 3, 0) && !board_get_anchor(board, 3, 3, 1));
  assert(board_get_anchor(board, 12, 12, 0) && board_get_anchor(board, 12, 12, 1));
