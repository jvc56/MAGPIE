...
void test_board_cross_set_for_player_index(Game *game, int player_index) {
  // Test cross set
  ...
  uint64_t cs = get_cross_set(game->gen->board, 0, 0,
                              BOARD_HORIZONTAL_DIRECTION, player_index);
  assert(!allowed(cs, 1));
  assert(allowed(cs, 0));
...
void test_board(SuperConfig *superconfig) {
  ...
  test_board_cross_set_for_player_index(game, 0);
  test_board_cross_set_for_player_index(game, 1);
  destroy_game(game);
}