...
void test_gen_cross_set(Game *game, int row, int col, int dir,
                        const char *letters, int expected_cross_score,
                        int run_gcs) {
  if (run_gcs) {
    gen_cross_set(game->gen->board, row, col, dir, game->gen->kwg,
...
void test_gen_cross_set_row(Game *game, int row, int col, int dir,
                            ...
                            int expected_cross_score, int run_gcs) {
  set_row(game, row, row_content);
  test_gen_cross_set(game, row, col, dir, letters, expected_cross_score,
                     run_gcs);
}

void test_gen_cross_set_col(Game *game, int row, int col, int dir,
                            const char *col_content, const char *letters,
                            int expected_cross_score, int run_gcs) {
...
void test_cross_set(SuperConfig *superconfig) {
  ...
  test_gen_cross_set_row(game, 4, 14, 0, "       Z Z Y A ", "ABDEGHILMNRSTWXY",
                         1, 1);
  ...
  gen_cross_set(game->gen->board, 7, 10, BOARD_HORIZONTAL_DIRECTION,
                game->gen->kwg, game->gen->letter_distribution);
  transpose(game->gen->board);
  gen_cross_set(game->gen->board, 10, 7, BOARD_VERTICAL_DIRECTION,
                game->gen->kwg, game->gen->letter_distribution);
  transpose(game->gen->board);
