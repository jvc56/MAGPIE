...
rack_to_sorted_sig(const Rack *rack, const LetterDistribution *ld, int ld_size,
                   char out[8]) {
  int idx = 0;
...
void test_generate_peg_cgps(void) {
  ...
  const char *out_path = out_env && *out_env ? out_env : "/tmp/random_peg.txt";
...
void test_pass_peg_print_report(void) {
  ...
  char **pass_cgps = NULL;
  char **rand_cgps = NULL;
  int n_pass = load_cgp_lines("/tmp/passpeg_candidates.txt", &pass_cgps, max_n);
...
static int peg_cascade_apply_quota(const PegCascadeRank *arr, int n, int n_cap,
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (take) {
      ...
      int wrote = snprintf(out_filter + off, out_filter_sz - off, "%s%s", sep,
                           r->cand_text);
...
static int peg_cascade_merge_rankings(const PegCascadeRank *prev, int prev_n,
                                      const PegCascadeRank *last, int last_n,
                                      PegCascadeRank **out_merged) {
  PegCascadeRank *out = malloc_or_die((size_t)(prev_n + last_n) * sizeof(*out));
...
void test_pass_peg_cascade(void) {
  ...
  const StageSpec stages[] = {
      ...
      {1, 32, 8, 8, 16},  {2, 16, 4, 4, 8}, {3, 8, 0, 0, 0}, {4, 4, 0, 0, 0},
  };
  ...
  const char *const overridden[] = {
      "PASSPEG_GREEDY_PATH",
      "PASSPEG_GREEDY_DEPTH",
      "PASSPEG_GREEDY_TOP_K",
      "PASSPEG_GREEDY_ONLY",
      "PASSPEG_GREEDY_RATIONAL",
      "PASSPEG_GREEDY_RAT_WALK",
      "PASSPEG_GREEDY_WALK_K_FIRST_OPP",
      "PASSPEG_GREEDY_WALK_K_LATER_OPP",
      "PASSPEG_SCENARIO_STRIDE",
      "PASSPEG_PERCEPTION_STRIDE",
      "PASSPEG_WALK_STRIDE",
      "PASSPEG_OPP_RANK_BY_PLAYOUT",
      "PASSPEG_OPP_RANK_POOL",
      "PASSPEG_GREEDY_BUDGET",
      "PASSPEG_GREEDY_THREADS",
      "PASSPEG_GREEDY_RESULT_FILE",
  ...
  for (int stage_idx = 0; stage_idx <= max_stage && stage_idx < n_stages;
       stage_idx++) {
    ...
    // Promote previous last_arr → prev_arr (it'stage_idx the basis of the
    // filter that just ran), then load the new stage'stage_idx results into
    // last_arr.
    free(prev_arr);
    prev_arr = last_arr;
    prev_n = last_n;
...
void test_peg_2bag_axe(void) {
  ...
  peg_assert_cand("peg2_axe", cgp, "6F (A)X(E)", 3, 0.9722, 41.292, 0.005, 2.0);
}
...
void test_peg_2bag_acidotic(void) {
  ...
  peg_assert_cand("peg2_acidotic", cgp, "C6 ACIDOT(I)c", 4, 1.0, 112.444, 0.005,
                  0.5);
}
...
static void peg_pess_eval_one(const MachineLetter *draw, int n, int64_t weight,
                              void *user) {
  ...
  const int32_t mover_lead =
      equity_to_int(
          player_get_score(game_get_player(scenario, pc->mover_idx))) -
      equity_to_int(player_get_score(game_get_player(scenario, pc->opp_idx)));
...
void test_pass_peg_pessimistic_eval(void) {
  ...
  const double win_pct = accum.orderings > 0
                             ? (double)accum.wins_x2 / (2.0 * accum.orderings)
                             : 0.0;
  const double mean_spread =
