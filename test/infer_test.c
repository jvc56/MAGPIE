...
error_code_t infer_for_test(InferenceCtx **ctx, const Config *config,
  ...
  infer_args_fill(
      ...
      config_get_num_threads(config), config_get_movegen_cache(config), 0,
      config_get_print_interval(config), config_get_thread_control(config),
      false, true, target_index, int_to_equity(target_score), target_num_exch,
