...
static void run_timed_precheck_ab(int num_games, double time_limit_sec,
  ...
  printf("  %4s  %6s  %7s %7s  %4s %4s  %7s %7s  %5s  %-8s %-8s"
         "  %7s %7s %5s %5s  %-24s\n",
         "Game", "Delta", "OldT1", "NewT1", "OD1", "ND1", "OldTot",
         "NewTot", "Trns", "P1", "P2", "OLook", "NLook", "OHit%",
         "NHit%", "Cumulative");
  printf("  ----  ------  ------- -------  ---- ----  ------- -------"
         "  -----  -------- --------"
         "  ------- ------- ----- -----  ------------------------\n");
  ...
  for (int gi = 0; games_played < num_games; gi++) {
    ...
    char rack_str0[32] = {0};
    char rack_str1[32] = {0};
    snprintf(rack_str0, sizeof(rack_str0), "%s", string_builder_peek(sb0));
    snprintf(rack_str1, sizeof(rack_str1), "%s", string_builder_peek(sb1));
    ...
    printf("  %4d  %+5d  %6.2fs %6.2fs  %4d %4d  %6.1fs %6.1fs  %3d/%d  "
           ...
           first_turn_depth[0], first_turn_depth[1], game_time[0],
           game_time[1], turn_count[0], turn_count[1], rack_str0,
           rack_str1, tt_lookups[0] / 1e6, tt_lookups[1] / 1e6, ohit_pct,
           nhit_pct, new_better, old_better, same_count, total_delta);
    (void)fflush(stdout);
  }
  ...
  printf("  Results (%d games from %d attempts, %.1fs/turn, seed=%llu):\n",
         games_played, attempts, time_limit_sec,
  ...
  printf("    Old T1 total: %.2fs (avg %.2fs)  |  New T1 total: %.2fs "
         "(avg %.2fs)\n",
         total_t1_old,
         games_played > 0 ? total_t1_old / games_played : 0.0, total_t1_new,
         games_played > 0 ? total_t1_new / games_played : 0.0);
...
// 4-way round robin tournament: head-to-head pairings with side-swapping.
// For each seed position, plays all C(4,2)=6 pairings twice (swap sides).
...
//
// Time management: each player has a total time budget for the entire endgame.
// Baseline: allocate 80% of remaining budget as hard limit per turn.
// Flexible: soft=60%, hard=90% with EBF-based prediction to decide whether
//           to start the next IDS depth.
//
// Configs: BO=baseline/precheck-off  FO=flexible/precheck-off
...
static void run_timed_round_robin(const char *cgp_file, int start_game,
  ...
  int cumul_net[6] = {0, 0, 0, 0, 0, 0};
  int games_played = 0;
  GameStringOptions *gso = game_string_options_create_default();

  // Overtime tracking: cumulative seconds over budget, per config
  double cumul_overtime[4] = {0.0, 0.0, 0.0, 0.0};
  ...
  for (int gi = 0; gi < num_games; gi++) {
    ...
    for (int pi = 0; pi < num_pairings; pi++) {
      ...
      for (int dir = 0; dir < 2; dir++) {
        ...
        while (game_get_game_end_reason(game) == GAME_END_REASON_NONE &&
          ...
          int cfg = cfg_for_player[pot];

          // Compute turn time limit from remaining budget.
          // Subtract 50ms from internal soft/hard limits to leave headroom
          // for the ~50ms timer-thread join that gets charged to wall clock.
          const double slack = 0.05;
          double turn_limit;
          double soft_limit = 0;
          double hard_limit = 0;
          // Halving floor: 500ms on first turn, halves each player turn,
          // minimum 20ms. Ensures a minimum search even when budget is low.
          const double floor_min = 0.020;
          double floor_secs = 0.500 / (double)(1 << player_turn_count[pot]);
          if (floor_secs < floor_min) floor_secs = floor_min;
          // Reserve floor_min for every remaining turn after this one so
          // future turns are never starved below the floor.
          ...
          double spendable = budget[pot] - reserved;
          if (spendable < floor_secs) spendable = floor_secs;
          if (cfg_time_mode[cfg] == 0) {
            // Baseline: 80% of spendable budget as hard limit
            turn_limit = spendable * 0.80;
          } else {
            // Flexible: EBF-based time management inside the solver
            // soft = 60% of spendable, hard = 90% of spendable (both with slack)
            soft_limit = spendable * 0.60 - slack;
            hard_limit = spendable * 0.90 - slack;
            if (soft_limit < 0) soft_limit = 0;
            if (hard_limit < 0) hard_limit = 0;
            turn_limit = spendable * 0.90; // external timer: no slack
          }
          // Apply floor
          if (turn_limit < floor_secs) turn_limit = floor_secs;
          ...
          err = error_stack_create();
          endgame_solve(solvers[pot], &args, res[pot], err);
          ...
          int depth =
              endgame_results_get_depth(res[pot], ENDGAME_RESULT_BEST);

          const PVLine *pv =
              endgame_results_get_pvline(res[pot], ENDGAME_RESULT_BEST);
          if (pv->num_moves == 0) {
            break;
          }
          ...
          snprintf(player_label, sizeof(player_label), "P%d(%s)",
                   pot + 1, cfg_names[cfg]);
        ...
        for (int p = 0; p < 2; p++) {
          if (budget[p] < 0) {
            ...
            if (p == 0) s0 -= penalty; else s1 -= penalty;
            cumul_overtime[cfg_for_player[p]] += ot;
          }
        }
        spread[dir] = s0 - s1;
        ...
        for (int i = 0; i < 2; i++) {
          endgame_results_destroy(res[i]);
          endgame_solver_destroy(solvers[i]);
        }
      }
    ...
    printf("  Game %d summary:", gi + 1);
    for (int pi = 0; pi < num_pairings; pi++) {
      printf("  %s=%+d", pair_labels[pi], net[pi]);
    }
    printf("\n");
    (void)fflush(stdout);
  }
  ...
  printf("=================================================================="
         "======================================\n");
  printf("  Results (%d games, P1=%.0fs P2=%.0fs):\n",
         games_played, p1_budget_sec, p2_budget_sec);
  printf("    Head-to-head net spread (+ = first config stronger):\n");
  for (int pi = 0; pi < num_pairings; pi++) {
    printf("      %s: %+d (avg %+.2f)\n", pair_labels[pi], cumul_net[pi],
           games_played > 0 ? (double)cumul_net[pi] / games_played : 0.0);
  }
  printf("    Overtime (total seconds over budget, 10pts/sec penalty):\n");
  for (int ci = 0; ci < 4; ci++) {
    printf("      %s: %.2fs\n", cfg_names[ci], cumul_overtime[ci]);
  }
  printf("=================================================================="
         "======================================\n");
  (void)fflush(stdout);
...
static void depth_log_callback(int depth, int32_t value,
  ...
  if (pv_line->num_moves > 0) {
    ...
    snprintf(move_str, sizeof(move_str), "%s", string_builder_peek(sb));
    string_builder_destroy(sb);
  }

  // Query actual root move count from solver
  int cur_depth_ignored = 0, root_done_ignored = 0, root_total = 0;
  int p2_done_ignored = 0, p2_total_ignored = 0;
  endgame_solver_get_progress(ctx->solver, &cur_depth_ignored,
                              &root_done_ignored, &root_total,
                              &p2_done_ignored, &p2_total_ignored);

  const char *cfg_name = ctx->config_idx == 0 ? "nopc" : "pc";
  (void)fprintf(ctx->log_fp,
                "%d,%d,%s,%d,P%d,d,%d,%.4f,%d,%d,,,,%s,,\n",
                ctx->game_id, ctx->dir, cfg_name, ctx->turn_num,
                ctx->player_on_turn + 1, depth, elapsed, value,
                root_total, move_str);
...
static void *poll_thread_func(void *arg) {
  ...
  while (!ctx->done) {
    ...
    (void)fprintf(ctx->log_fp,
                  ...
                  ctx->game_id, ctx->dir, cfg_name, ctx->turn_num,
                  ctx->player_on_turn + 1, cur_depth, elapsed,
                  root_done, root_total,
                  lookups, hits, created,
                  p2_done, p2_total);
  ...
  for (int gi = 0; games_played < num_games; gi++) {
    ...
    for (int dir = 0; dir < 2; dir++) {
      ...
      printf("  G%d: P1=%s P2=%s\n", dir + 1,
             label_for_player[0], label_for_player[1]);
      ...
      while (game_get_game_end_reason(game) == GAME_END_REASON_NONE &&
        ...
        double turn_limit = budget[pot] * 0.80;
        if (turn_limit < 0.1) {
          turn_limit = 0.1;
        }

        depth_ctx.config_idx = skip_precheck_for_player[pot] ? 0 : 1;
        depth_ctx.turn_num = turns;
        depth_ctx.player_on_turn = pot;
        depth_ctx.solver = solvers[pot];
        depth_ctx.turn_limit_s = turn_limit;
        ctimer_start(&depth_ctx.turn_timer);

        const char *cfg_name_start =
            depth_ctx.config_idx == 0 ? "nopc" : "pc";
        (void)fprintf(log_fp, "%d,%d,%s,%d,P%d,start,,,,,,,,,,%.4f,\n",
                      games_played, dir, cfg_name_start, turns, pot + 1,
                      turn_limit);
        ...
        PollContext poll_ctx = {
            .turn_timer = &depth_ctx.turn_timer,
            .log_fp = log_fp,
            .solver = solvers[pot],
            .results = results[pot],
        ...
        ctimer_start(&t);
        err = error_stack_create();
        endgame_solve(solvers[pot], &args, results[pot], err);
        double elapsed = ctimer_elapsed_seconds(&t);
        ...
        int timed_out =
            (thread_control_get_status(tc) ==
             THREAD_CONTROL_STATUS_USER_INTERRUPT)
                ? 1
                : 0;
        int end_depth =
            endgame_results_get_depth(results[pot], ENDGAME_RESULT_BEST);
        const char *cfg_name_end =
            depth_ctx.config_idx == 0 ? "nopc" : "pc";
        (void)fprintf(log_fp, "%d,%d,%s,%d,P%d,end,%d,%.4f,,,,,,,,,%d\n",
                      games_played, dir, cfg_name_end, turns, pot + 1,
                      end_depth, elapsed, timed_out);
        (void)fflush(log_fp);
        ...
        const PVLine *pv =
            endgame_results_get_pvline(results[pot], ENDGAME_RESULT_BEST);
        ...
        printf("    t%d P%d(%s) d%d %.2fs bud=%.1f %s\n",
               turns, pot + 1, label_for_player[pot], depth, elapsed,
               budget[pot], move_str);
      ...
      for (int i = 0; i < 2; i++) {
        endgame_results_destroy(results[i]);
        endgame_solver_destroy(solvers[i]);
      }
    }
    ...
    printf("  Net: %+d  (cumul %+d, pc+%d nopc+%d =%d)\n",
           net, total_net, pc_better, nopc_better, same_count);
    (void)fflush(stdout);
    (void)fflush(log_fp);
  }
