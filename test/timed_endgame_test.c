...
static void run_profiling_workload(int num_games, uint64_t base_seed) {
  const int max_ply = 25;
  const double tt_frac = 0.25;
  const double p1_budget_init = 2.0;
  const double p2_budget_init = 1.2;
  const int max_turns = 50;
  ...
  printf("Profiling workload: games=%d  seed=%llu\n", num_games,
         (unsigned long long)base_seed);
  (void)fflush(stdout);
  ...
  for (; games_played < num_games;) {
    ...
    double budget[2] = {p1_budget_init, p2_budget_init};
    int player_turn_count[2] = {0, 0};
    int turn_num = 0;
    ...
    while (game_get_game_end_reason(game) == GAME_END_REASON_NONE &&
      ...
      ta.done = true;
      pthread_join(timer_tid, NULL);
      budget[pot] -= ctimer_elapsed_seconds(&turn_timer);
      assert(error_stack_is_empty(err));
      error_stack_destroy(err);
    ...
    for (int pp = 0; pp < 2; pp++) {
      endgame_results_destroy(res[pp]);
      endgame_solver_destroy(solvers[pp]);
    }
...
void test_benchmark_profile_precheck(void) {
  log_set_level(LOG_FATAL);
  run_profiling_workload(500, 31415926);
}
