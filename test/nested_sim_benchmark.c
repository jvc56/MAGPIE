...
static void print_sim_results(const char *label, SimResults *sim_results,
  ...
  for (int i = 0; i < num_plays && i < 10; i++) {
    ...
    printf("  %-4d %-20s %7.2f%% %8.2f %8lu\n", i + 1, string_builder_peek(sb),
           win_pct, eq, (unsigned long)iters);
  }
  string_builder_destroy(sb);
}

static void run_benchmark(Config *config, const char *label,
                          int num_fidelity_levels, const FidelityLevel *levels,
                          int num_threads) {
  ...
  config_simulate(config, NULL, NULL, sim_results, NULL, 0, error_stack);
...
void test_nested_sim_benchmark(void) {
  ...
  run_benchmark(config, "MULTI-FIDELITY (5s static + 10s nested)", 2, mf_levels,
                num_threads);
