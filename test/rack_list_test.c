...
void test_rack_list(void) {
  ...
  RackList *rack_list = rack_list_create(ld, 3, NULL, 0, error_stack);
...
void test_rack_list_forced_racks(void) {
  ...
  // A duplicate forced rack must be rejected rather than silently
  ...
  const RackList *duplicate_rack_list =
      rack_list_create(ld, 3, duplicate_racks, 3, duplicate_error_stack);
  ...
  // A forced rack that isn't a full rack must be rejected too.
  const char *malformed_racks[] = {"AAAAAAB", "AB"};
  ErrorStack *malformed_error_stack = error_stack_create();
  const RackList *malformed_rack_list =
      rack_list_create(ld, 3, malformed_racks, 2, malformed_error_stack);
  assert(malformed_rack_list == NULL);
  assert(error_stack_top(malformed_error_stack) ==
         ERROR_STATUS_AUTOPLAY_FORCE_RACKS_MALFORMED_RACK);
  error_stack_destroy(malformed_error_stack);

  // Distinct forced racks restrict the rare partition to just those racks,
  // and rack_list_get_rack_equity_json reports every observed rack, not
  // just the forced ones.
  const char *forced_racks[] = {"AAAAAAB", "ABBBBBB"};
  ErrorStack *error_stack = error_stack_create();
  RackList *rack_list = rack_list_create(ld, 3, forced_racks, 2, error_stack);
  assert(error_stack_is_empty(error_stack));
  error_stack_destroy(error_stack);
  ...
  // Also record a non-forced rack: it should show up in the JSON, but never
