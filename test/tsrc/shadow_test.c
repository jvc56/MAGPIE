...
void test_shadow_score() {
  ...
  /*
    ...
    assert(
        within_epsilon(anchor_get_highest_possible_equity(anchor_list, 0), 10));
    // EKE
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 1),
    9));
    // KAEE
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 2),
    8));
    // EE and E(A)
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 3),
    5));
    // EE and E(A)
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 4),
    5));
    // EEE
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 5),
    3));
    ...
    7));
    // Ea, EA
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 6),
    3));
    // Ae, eE
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 7),
    3));
    // E(A)a
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 8),
    2));
    ...
    assert(
        within_epsilon(anchor_get_highest_possible_equity(anchor_list, 0),
    102));

    load_and_generate(game, move_list, player, DOUG_V_EMELY_CGP, "Q");
    // WINDY is not extendable, so there is no 22 for WINDYQ.
    // The highest anchor is through the W in WINDY, recording 14 for QW, as W
    is
    ...
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 0),
    6)); assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list,
    1), 6));
    assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list, 3),
    6)); assert(within_epsilon(anchor_get_highest_possible_equity(anchor_list,
    4), 5));
    ...
    " "0 lex CSW21"; load_and_generate(game, move_list, player, qi_qis,
    "FRUITED"); AnchorList *al = gen_get_anchor_list(0);
    // 8g (QI)DURFITE 128
    // h8 (I)DURFITE 103
    // f9 UFTRIDE 88
    // 7g EF(QIS)RTUDI 79
