...
// Measures a lexicon's PAT opening table by simulation: for seeded opening
// racks, the top PAT_OPENING_SIM_NUM_PLAYS static candidates under the given
// weights are each simmed PAT_OPENING_SIM_PLIES plies for
...
// averaged by the candidate's tile count, is what static evaluation misses
// about opening length; the table fitted on the dev racks (even attempts) is
// scored on the eval racks (odd attempts) by the paired difference
// sim(choice with table) - sim(static choice), which is unbiased because
// neither choice looks at the sim noise -- unlike a regret against the
// sim-best play, which the max over noisy means inflates.
//
// Measure a lexicon's table under weights WITHOUT an opening table, or the
// gap would be measured against a static equity that already carries it,
