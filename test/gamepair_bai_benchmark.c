...
void test_gamepair_bai_benchmark(void) {
  ...
  for (int pair = 0; pair < NUM_GAME_PAIRS; pair++) {
    ...
    play_game(game, move_list, sim_results, &sim_ctx, win_pcts, tc,
              &endgame_ctx, endgame_results, &STRATEGY_STATIC, &STRATEGY_NESTED,
              STATIC_PLIES, NESTED_PLIES, "STATIC", "NESTED", pair * 2 + 1,
              seed, &static_as_p0);

    // Game B: Nested=P0, Static=P1 (same seed, swapped strategies)
    play_game(game, move_list, sim_results, &sim_ctx, win_pcts, tc,
              &endgame_ctx, endgame_results, &STRATEGY_NESTED, &STRATEGY_STATIC,
              NESTED_PLIES, STATIC_PLIES, "NESTED", "STATIC", pair * 2 + 2,
              seed, &nested_as_p0);
  }
