...
void pat_run_through_table_check(const char *lexicon) {
  ...
  for (int r = 0; r < 5; r++) {
    ...
    MachineLetter key[3] = {0};
    for (int k = 0; k < key_len; k++) {
      const char one[2] = {runs[r][k], 0};
...
static void pat_path_parity_run(double utility_adjust, double cap_points) {
  ...
  for (int c = 0; c < 2; c++) {
    const Game *game = config_get_game(configs[c]);
...
static void test_pat_usage_options(void) {
  ...
  assert(!players_data_get_pat_rollout_disabled(players_data, 1));
  for (int player_index = 0; player_index < 2; player_index++) {
    assert(players_data_get_pat_disabled_classes_mask(players_data,
                                                      player_index) ==
           (PAT_CLASS_MASK_ALL &
            ~((1U << PAT_PREMIUM_DWS) | (1U << PAT_PREMIUM_TWS))));
  }
...
static void test_pat_stage_scale(const char *data_dir) {
  ...
  for (int attempt = 0; attempt < 12; attempt++) {
    ...
    if (stage != PAT_STAGE_EARLY) {
      ...
      const MoveList *expected_list =
          (stage == PAT_STAGE_MID) ? lists[1] : lists[0];
      for (int a = 0; a < num; a++) {
