    ...
    */
    ...
    /*
        if (!CORE.Window.fullscreen)
        {
            ...
            TRACELOG(RAYLIB_LOG_WARNING, "Emscripten: Enter fullscreen: Canvas size: %i x %i", width, height);
        ...
        else
        {
            ...
            int width, height;
            emscripten_get_canvas_element_size("#canvas", &width, &height);
            TRACELOG(RAYLIB_LOG_WARNING, "Emscripten: Exit fullscreen: Canvas size: %i x %i", width, height);
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "MinimizeWindow() not available on target platform");
}
...
void SetWindowState(unsigned int flags)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_VSYNC_HINT) not available on target platform");
    }
    ...
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_UNDECORATED) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIDDEN
    if ((flags & FLAG_WINDOW_HIDDEN) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_HIDDEN) not available on target platform");
    }

    // State change: FLAG_WINDOW_MINIMIZED
    if ((flags & FLAG_WINDOW_MINIMIZED) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_MINIMIZED) not available on target platform");
    }
    ...
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_UNFOCUSED) not available on target platform");
    }

    // State change: FLAG_WINDOW_TOPMOST
    if ((flags & FLAG_WINDOW_TOPMOST) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_TOPMOST) not available on target platform");
    }

    // State change: FLAG_WINDOW_ALWAYS_RUN
    if ((flags & FLAG_WINDOW_ALWAYS_RUN) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_ALWAYS_RUN) not available on target platform");
    }
    ...
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_TRANSPARENT) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIGHDPI
    if ((flags & FLAG_WINDOW_HIGHDPI) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_HIGHDPI) not available on target platform");
    }

    // State change: FLAG_WINDOW_MOUSE_PASSTHROUGH
    if ((flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_WINDOW_MOUSE_PASSTHROUGH) not available on target platform");
    }

    // State change: FLAG_MSAA_4X_HINT
    if ((flags & FLAG_MSAA_4X_HINT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_MSAA_4X_HINT) not available on target platform");
    }

    // State change: FLAG_INTERLACED_HINT
    if ((flags & FLAG_INTERLACED_HINT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "SetWindowState(FLAG_INTERLACED_HINT) not available on target platform");
    }
}
...
void ClearWindowState(unsigned int flags)
{
    ...
    if ((flags & FLAG_VSYNC_HINT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_VSYNC_HINT) not available on target platform");
    }
    ...
    if ((flags & FLAG_WINDOW_HIDDEN) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_HIDDEN) not available on target platform");
    }

    // State change: FLAG_WINDOW_MINIMIZED
    if ((flags & FLAG_WINDOW_MINIMIZED) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_MINIMIZED) not available on target platform");
    }
    ...
    if ((flags & FLAG_WINDOW_UNDECORATED) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_UNDECORATED) not available on target platform");
    }

    // State change: FLAG_WINDOW_UNFOCUSED
    if ((flags & FLAG_WINDOW_UNFOCUSED) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_UNFOCUSED) not available on target platform");
    }

    // State change: FLAG_WINDOW_TOPMOST
    if ((flags & FLAG_WINDOW_TOPMOST) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_TOPMOST) not available on target platform");
    }

    // State change: FLAG_WINDOW_ALWAYS_RUN
    if ((flags & FLAG_WINDOW_ALWAYS_RUN) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_ALWAYS_RUN) not available on target platform");
    }
    ...
    if ((flags & FLAG_WINDOW_TRANSPARENT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_TRANSPARENT) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIGHDPI
    if ((flags & FLAG_WINDOW_HIGHDPI) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_HIGHDPI) not available on target platform");
    }

    // State change: FLAG_WINDOW_MOUSE_PASSTHROUGH
    if ((flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_WINDOW_MOUSE_PASSTHROUGH) not available on target platform");
    }

    // State change: FLAG_MSAA_4X_HINT
    if ((flags & FLAG_MSAA_4X_HINT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_MSAA_4X_HINT) not available on target platform");
    }

    // State change: FLAG_INTERLACED_HINT
    if ((flags & FLAG_INTERLACED_HINT) > 0)
    {
        TRACELOG(RAYLIB_LOG_WARNING, "ClearWindowState(FLAG_INTERLACED_HINT) not available on target platform");
    }
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "SetWindowIcon() not available on target platform");
}

// Set icon for window, multiple images
void SetWindowIcons(Image *images, int count)
{
    TRACELOG(RAYLIB_LOG_WARNING, "SetWindowIcons() not available on target platform");
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "SetWindowPosition() not available on target platform");
}

// Set monitor for the current window
void SetWindowMonitor(int monitor)
{
    TRACELOG(RAYLIB_LOG_WARNING, "SetWindowMonitor() not available on target platform");
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "SetWindowFocused() not available on target platform");
}

// Get native window handle
void *GetWindowHandle(void)
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetWindowHandle() not implemented on target platform");
    return NULL;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorCount() not implemented on target platform");
    return 1;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetCurrentMonitor() not implemented on target platform");
    return 0;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorPosition() not implemented on target platform");
    return (Vector2){ 0, 0 };
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorPhysicalWidth() not implemented on target platform");
    return 0;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorPhysicalHeight() not implemented on target platform");
    return 0;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorRefreshRate() not implemented on target platform");
    return 0;
}
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetMonitorName() not implemented on target platform");
    return "";
}
...
void SetClipboardText(const char *text)
{
    ...
    if (strchr(text, '\'') != NULL) TRACELOG(RAYLIB_LOG_WARNING, "SYSTEM: Provided Clipboard could be potentially malicious, avoid [\'] character");
...
Image GetClipboardImage(void)
{
    ...
    TRACELOG(RAYLIB_LOG_WARNING, "GetClipboardImage() not implemented on target platform");
...
void OpenURL(const char *url)
{
    ...
    if (strchr(url, '\'') != NULL) TRACELOG(RAYLIB_LOG_WARNING, "SYSTEM: Provided URL could be potentially malicious, avoid [\'] character");
...
{
    TRACELOG(RAYLIB_LOG_INFO, "SetGamepadMappings not implemented in rcore_web.c");
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GetKeyName() not implemented on target platform");
    return "";
}
...
int InitPlatform(void)
{
    ...
    int result = glfwInit();
    if (result == GLFW_FALSE) { TRACELOG(RAYLIB_LOG_WARNING, "GLFW: Failed to initialize GLFW"); return -1; }
    ...
    if (CORE.Window.flags & FLAG_MSAA_4X_HINT)
    {
        ...
        TRACELOG(RAYLIB_LOG_INFO, "DISPLAY: Trying to enable MSAA x4");
    ...
    if (CORE.Window.fullscreen)
    {
        ...
        TRACELOG(RAYLIB_LOG_WARNING, "SYSTEM: Closest fullscreen videomode: %i x %i", CORE.Window.display.width, CORE.Window.display.height);
    ...
    {
        glfwTerminate();
        TRACELOG(RAYLIB_LOG_WARNING, "GLFW: Failed to initialize Window");
        return -1;
    }
    ...
    if (result == true) //(result != GLFW_NO_WINDOW_CONTEXT) && (result != GLFW_PLATFORM_ERROR))
    {
        ...
        TRACELOG(RAYLIB_LOG_INFO, "DISPLAY: Device initialized successfully");
        TRACELOG(RAYLIB_LOG_INFO, "    > Display size: %i x %i", CORE.Window.display.width, CORE.Window.display.height);
        TRACELOG(RAYLIB_LOG_INFO, "    > Screen size:  %i x %i", CORE.Window.screen.width, CORE.Window.screen.height);
        TRACELOG(RAYLIB_LOG_INFO, "    > Render size:  %i x %i", CORE.Window.render.width, CORE.Window.render.height);
        TRACELOG(RAYLIB_LOG_INFO, "    > Viewport offsets: %i, %i", CORE.Window.renderOffset.x, CORE.Window.renderOffset.y);
    }
    else
    {
        TRACELOG(RAYLIB_LOG_FATAL, "PLATFORM: Failed to initialize graphics device");
        return -1;
    }
    ...
    if (!CORE.Window.ready) { TRACELOG(RAYLIB_LOG_FATAL, "PLATFORM: Failed to initialize graphic device"); return -1; }
    ...
    TRACELOG(RAYLIB_LOG_INFO, "PLATFORM: WEB: Initialized successfully");
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "GLFW: Error: %i Description: %s", error, description);
}
