...
void TraceLog(int logType, const char *text, ...)
{
    ...
    switch (logType)
    {
        ...
        case RAYLIB_LOG_DEBUG: __android_log_vprint(ANDROID_RAYLIB_LOG_DEBUG, "raylib", text, args); break;
        case RAYLIB_LOG_INFO: __android_log_vprint(ANDROID_RAYLIB_LOG_INFO, "raylib", text, args); break;
        case RAYLIB_LOG_WARNING: __android_log_vprint(ANDROID_LOG_WARN, "raylib", text, args); break;
        case RAYLIB_LOG_ERROR: __android_log_vprint(ANDROID_RAYLIB_LOG_ERROR, "raylib", text, args); break;
        case RAYLIB_LOG_FATAL: __android_log_vprint(ANDROID_RAYLIB_LOG_FATAL, "raylib", text, args); break;
        default: break;
    }
#else
        ...
        if (file != NULL)
        {
            ...
            if (size > 0)
            {
                ...
                if (data != NULL)
                {
                    ...
                    if (count > 2147483647)
                    {
                        TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] File is bigger than 2147483647 bytes, avoid using LoadFileData()", fileName);
                    ...
                    else
                    {
                        *dataSize = (int)count;
                        ...
                        if ((*dataSize) != size) TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] File partially loaded (%i bytes out of %i)", fileName, dataSize, count);
                        else TRACELOG(RAYLIB_LOG_INFO, "FILEIO: [%s] File loaded successfully", fileName);
                    }
                }
                else TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] Failed to allocated memory for file reading", fileName);
            }
...
#else
    TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: Standard file io not supported, use custom file callback");
#endif
    }
...
bool ExportDataAsCode(const unsigned char *data, int dataSize, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(RAYLIB_LOG_INFO, "FILEIO: [%s] Data as code exported successfully", fileName);
    else TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] Failed to export data as code", fileName);
        ...
        if (file != NULL)
        {
            ...
            if (size > 0)
            {
                ...
                if (text != NULL)
                {
                    ...
                    TRACELOG(RAYLIB_LOG_INFO, "FILEIO: [%s] Text file loaded successfully", fileName);
                }
                else TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] Failed to allocated memory for file reading", fileName);
            }
            else TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] Failed to read text file", fileName);

            fclose(file);
        }
        else TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: [%s] Failed to open text file", fileName);
#else
    TRACELOG(RAYLIB_LOG_WARNING, "FILEIO: Standard file io not supported, use custom file callback");
#endif
    }
...
{
    TRACELOG(RAYLIB_LOG_WARNING, "ANDROID: Failed to provide write access to APK");
