 ...
 * domain). Do not edit: update by replacing this file. */
#include "sha256.h"
...
void sha256_final(SHA256_CTX *ctx, uint8_t hash[SHA256_BLOCK_SIZE]) {
  ...
  /* Big-endian output, which is what the hex digest everyone quotes is. */
  for (i = 0; i < 4; ++i) {
    for (uint32_t s = 0; s < 8; ++s) {
      hash[i + (s * 4)] = (uint8_t)((ctx->state[s] >> (24 - i * 8)) & 0x000000ff);
    }
  }
}
