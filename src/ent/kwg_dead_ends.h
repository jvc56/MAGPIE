...
static inline uint64_t get_next_dead_end_index(KWGDeadEnds *kwgde,
                                               uint64_t current_dead_end_index,
                                               uint8_t letter) {
  int letter_val = letter;
  if (letter == SEPARATION_MACHINE_LETTER) {
    letter_val = kwgde->base_offset - 1;
  }
  return current_dead_end_index +
         kwgde->dead_end_level_offsets[kwgde->dead_end_level - 1] * letter_val;
}

static inline void set_dead_end(KWGDeadEnds *kwgde, uint64_t dead_end_index) {
  kwgde->dead_ends[kwgde->number_of_dead_ends++] = dead_end_index;
}
        ...
        if (number_of_ml > 0) {
          ...
          acceptable_word_found |=
              kwgde_go_on(kwgde, ml, next_node_index, accepts,
                          already_switched_dir, current_dead_end_index);
          kwgde->tiles_played--;
          kwgde->dead_end_level--;
...
static inline bool kwgde_go_on(KWGDeadEnds *kwgde, uint8_t current_letter,
                               uint32_t new_node_index, bool accepts,
                               bool already_switched_dir,
                               uint64_t previous_dead_end_index) {
  ...
  uint64_t current_dead_end_index =
      get_next_dead_end_index(kwgde, previous_dead_end_index, current_letter);
  ...
  if (!already_switched_dir) {
    ...
    acceptable_word_found |= kwgde_recursive_gen(
        kwgde, new_node_index, already_switched_dir, current_dead_end_index);
    ...
    if (separation_node_index != 0) {
      ...
      acceptable_word_found |= kwgde_recursive_gen(
          ...
          get_next_dead_end_index(kwgde, current_dead_end_index,
  ...
  if (!acceptable_word_found && !rack_is_empty(&kwgde->rack)) {
    ...
    set_dead_end(kwgde, current_dead_end_index);
  }
