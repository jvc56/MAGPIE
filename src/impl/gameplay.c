...
// Versions of calc_for_across and calc_for_self that use an explicit KWG
// instead of the player's KWG. Used for endgame solving with pruned KWGs.
static void calc_for_across_with_kwg(const Move *move, const Game *game,
  ...
  for (int row = row_start; row < move_get_tiles_length(move) + row_start;
    ...
    const int left_col =
        board_get_word_edge(board, row, col_start, WORD_DIRECTION_LEFT);
    // Use the same KWG for both players' cross sets
    game_gen_cross_set_with_kwg(game, row, right_col + 1, csd, 0, kwg);
    game_gen_cross_set_with_kwg(game, row, left_col - 1, csd, 0, kwg);
    game_gen_cross_set_with_kwg(game, row, col_start, csd, 0, kwg);
    if (!game_get_data_is_shared(game, PLAYERS_DATA_TYPE_KWG)) {
      game_gen_cross_set_with_kwg(game, row, right_col + 1, csd, 1, kwg);
      game_gen_cross_set_with_kwg(game, row, left_col - 1, csd, 1, kwg);
      game_gen_cross_set_with_kwg(game, row, col_start, csd, 1, kwg);
    }
  }
}

static void calc_for_self_with_kwg(const Move *move, const Game *game,
                                   int row_start, int col_start, int csd,
                                   const KWG *kwg) {
  for (int col = col_start - 1; col <= col_start + move_get_tiles_length(move);
       col++) {
    game_gen_cross_set_with_kwg(game, row_start, col, csd, 0, kwg);
  }
  if (!game_get_data_is_shared(game, PLAYERS_DATA_TYPE_KWG)) {
    for (int col = col_start - 1;
         col <= col_start + move_get_tiles_length(move); col++) {
      game_gen_cross_set_with_kwg(game, row_start, col, csd, 1, kwg);
    }
  }
}
...
void update_cross_set_for_move_with_kwg(const Move *move, const Game *game,
  ...
  Board *board = game_get_board(game);
  if (board_is_dir_vertical(move_get_dir(move))) {
    calc_for_across_with_kwg(move, game, move_get_row_start(move),
                             move_get_col_start(move), BOARD_VERTICAL_DIRECTION,
                             kwg);
    board_transpose(board);
    calc_for_self_with_kwg(move, game, move_get_col_start(move),
                           move_get_row_start(move), BOARD_VERTICAL_DIRECTION,
                           kwg);
    board_transpose(board);
  } else {
    calc_for_self_with_kwg(move, game, move_get_row_start(move),
                           move_get_col_start(move), BOARD_VERTICAL_DIRECTION,
                           kwg);
    board_transpose(board);
    calc_for_across_with_kwg(move, game, move_get_col_start(move),
                             move_get_row_start(move), BOARD_VERTICAL_DIRECTION,
                             kwg);
    board_transpose(board);
  }
