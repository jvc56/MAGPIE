...
wmp_move_gen_save_playthrough_state(WMPMoveGen *wmp_move_gen) {
  ...
  wmp_move_gen->num_tiles_played_through_copy =
      wmp_move_gen->num_tiles_played_through;
  printf("saving playthrough blocks: %d\n", wmp_move_gen->playthrough_blocks);
...
wmp_move_gen_restore_playthrough_state(WMPMoveGen *wmp_move_gen) {
  ...
  wmp_move_gen->num_tiles_played_through =
      wmp_move_gen->num_tiles_played_through_copy;
  printf("restoring playthrough blocks, previous %d, making it %d\n",
         wmp_move_gen->playthrough_blocks_copy,
         wmp_move_gen->playthrough_blocks);
...
static inline void wmp_move_gen_maybe_update_anchor(WMPMoveGen *wmp_move_gen,
                                                    ...
                                                    Equity score,
                                                    Equity equity) {
  printf("maybe_update_anchor tiles_played=%d playthrough_blocks=%d\n",
         tiles_played, wmp_move_gen->playthrough_blocks);
...
wmp_move_gen_set_anchor_playthrough(WMPMoveGen *wmp_move_gen,
  ...
  printf("wmp_move_gen_set_anchor_playthrough, anchor: row %d col %d dir "
         "%s tiles %d blocks %d\n",
         anchor->row, anchor->col, anchor->dir ? "vertical" : "horizontal",
         anchor->tiles_to_play, anchor->playthrough_blocks);
  printf(" row_cache: [");
  for (int i = 0; i < BOARD_DIM; i++) {
    char c = '.';
    if (square_get_letter(&row_cache[i]) != ALPHABET_EMPTY_SQUARE_MARKER) {
      c = square_get_letter(&row_cache[i]) + 'A' - 1;
    }
    printf("%c", c);
  }
  printf("]\n");
...
static inline void wmp_move_gen_add_anchors(WMPMoveGen *wmp_move_gen, int row,
  ...
  printf("wmp_move_gen_add_anchors: row %d col %d last_anchor_col %d dir "
         "%d\n",
         row, col, last_anchor_col, dir);
  for (int i = 0; i < MAX_WMP_MOVE_GEN_ANCHORS; i++) {
    const Anchor *anchor = &wmp_move_gen->anchors[i];
    if (anchor->tiles_to_play > 0) {
      printf("Anchor %d: highest_possible_equity: %d, highest_possible_score: "
             "%d, tiles_to_play: %d, playthrough_blocks: %d\n",
             i, anchor->highest_possible_equity, anchor->highest_possible_score,
             anchor->tiles_to_play, anchor->playthrough_blocks);
