...
// Fallback ply-decision when the bag is empty. The original nested-sim branch
// ran a real 2-ply endgame solve here via EndgamePlyCtx; that lightweight
// solver hasn't been ported to main's new EndgameCtx/EndgameCtxWorker API yet,
// so for now we degrade to the same static eval used elsewhere in the rollout.
static const Move *endgame_ply_solve(Game *game, int thread_index,
                                     SimmerWorker *worker) {
  return get_top_equity_move(game, thread_index, worker->move_list);
}
...
double rv_sim_sample(RandomVariables *rvs, const uint64_t play_index,
  ...
  for (int ply = 0; ply < plies; ply++) {
    ...
    if (bag_is_empty(game_get_bag(game))) {
      ...
      best_play = endgame_ply_solve(game, thread_index, simmer_worker);
