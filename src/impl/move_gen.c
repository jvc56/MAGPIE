...
static inline double get_move_equity_for_sort_type_wmp(MoveGen *gen, Move *move,
  ...
  if (gen->move_sort_type == MOVE_SORT_EQUITY) {
    //printf("move score: %d\n", move_get_score(move));
    //printf("number_of_tiles_played: %d number_of_tiles_in_bag %d\n",
...
void record_wmp_play(MoveGen *gen, int start_col, double leave_value) {
  ...
  for (int letter_idx = 0; letter_idx < wgen->word_length; letter_idx++) {
    ...
    if (ml != PLAYED_THROUGH_MARKER) {
      ...
      const BonusSquare bonus_square = gen_cache_get_bonus_square(gen, board_col);
      const int this_word_multiplier = bonus_square_get_word_multiplier(bonus_square);
      ...
      const int is_cross_word = gen_cache_get_is_cross_word(gen, board_col) ? 1 : 0;
      const int letter_multiplier = bonus_square_get_letter_multiplier(bonus_square);
      played_cross_total +=
...
void shadow_play_for_anchor(MoveGen *gen, int col) {
  ...
  gen->highest_shadow_equity = EQUITY_INITIAL_VALUE;
  gen->highest_shadow_score = EQUITY_INITIAL_VALUE;
