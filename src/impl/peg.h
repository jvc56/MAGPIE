...
// Fired when a candidate finishes. nested_endgame_nodes is a solve-wide
// snapshot taken when the callback is delivered. It is an exact portable work
// boundary in the live/poll path, where refinement candidates run serially;
// callbacks delivered after the scenario-parallel stage barrier all see that
// barrier's final solve-wide count. completed_ns is its absolute
// monotonic-clock completion timestamp (ctimer_monotonic_ns-compatible).
// reordered is true when the candidate slotted in above the bottom of the live
// ranking (so the displayed order changed and the whole list should be
