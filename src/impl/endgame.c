  ...
  for (int p = start; p <= plies; p++) {
    ...
    if (worker->solver->aspiration_optim && p > 1 &&
      ...
      while (val <= asp_alpha || val >= asp_beta) {
        // Track nodes wasted on this failed search
        int nodes_this_search = worker->solver->nodes_searched - nodes_before;
        worker->solver->nodes_wasted_on_fails += nodes_this_search;
