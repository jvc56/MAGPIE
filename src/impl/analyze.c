...
typedef struct GenMovesResult {
  int heap_idx;
  Move actual_move_or_pass_if_phony;
} GenMovesResult;

static double move_equity_to_double(const Move *move) {
  if (move_get_type(move) == GAME_EVENT_PASS) {
    return EQUITY_PASS_DISPLAY_DOUBLE;
  }
...
static void add_move_comparison_rows(StringGrid *move_sg, const TurnResult *tr,
  ...
  for (int ply_idx = 0; ply_idx < num_display_plies; ply_idx++) {
    ...
    string_grid_set_cell(
        move_sg, 2, score_col,
        get_formatted_string("%.2f", actual_trm->ply_score[ply_idx]));
    string_grid_set_cell(
        move_sg, 2, bingo_col,
    ...
    string_grid_set_cell(
        move_sg, 3, bingo_col,
        get_formatted_string("%.2f", best_trm->ply_bingo_pct[ply_idx] -
                                         actual_trm->ply_bingo_pct[ply_idx]));
  }
...
static void write_per_turn_human_readable(
  ...
  for (int ply_idx = 0; ply_idx < num_display_plies; ply_idx++) {
    ...
    string_grid_set_cell(move_sg, curr_row, curr_col++,
                         get_formatted_string("P%d-BP", ply_idx + 1));
  }
...
Move analyze_get_move_or_pass_if_phony(const ValidatedMoves *vms) {
  if (validated_moves_is_phony(vms, 0)) {
    Move pass_move;
    move_set_as_pass(&pass_move);
    return pass_move;
  }
  return *validated_moves_get_move(vms, 0);
}
...
analyze_gen_moves_with_actual(AnalyzeCtx *ctx, const ValidatedMoves *vms,
  ...
  for (int move_idx = 0; move_idx < move_count; move_idx++) {
    if (compare_moves_without_equity(
            ...
            &actual_move_or_pass_if_phony, true) == -1) {
    ...
    if (vertical_opening_is_transposable &&
      ...
      move_copy(gen_move, &actual_move_or_pass_if_phony);
  ...
  move_list_add_move(ctx->move_list, &actual_move_or_pass_if_phony);
  return (GenMovesResult){.heap_idx = -1,
                          .actual_move_or_pass_if_phony =
                              actual_move_or_pass_if_phony};
}
...
static void analyze_with_static(const GameEvent *event, TurnResult *turn_result,
  ...
  for (int rank_idx = 0; rank_idx < sorted_count; rank_idx++) {
    if (compare_moves_without_equity(
            move_list_get_move(ctx->move_list, rank_idx),
            &gmr.actual_move_or_pass_if_phony, true) == -1) {
...
static void analyze_with_sim(const GameEvent *event, TurnResult *turn_result,
  ...
  for (int sp_idx = 0; sp_idx < num_moves; sp_idx++) {
    const SimmedPlay *sp =
        sim_results_get_display_simmed_play(ctx->sim_results, sp_idx);
    const Move *sp_move = simmed_play_get_move(sp);
    if (compare_moves_without_equity(sp_move, &gmr.actual_move_or_pass_if_phony,
                                     true) == -1) {
      actual_sp = sp;
  ...
  const int num_plies = sim_results_get_num_plies(ctx->sim_results);
  turn_result->actual.num_plies = num_plies;
  for (int ply_idx = 0; ply_idx < num_plies; ply_idx++) {
    turn_result->actual.ply_score[ply_idx] =
        stat_get_mean(simmed_play_get_score_stat(actual_sp, ply_idx));
    turn_result->actual.ply_bingo_pct[ply_idx] =
        stat_get_mean(simmed_play_get_bingo_stat(actual_sp, ply_idx)) * 100.0;
  }
  ...
  if (best_sp == actual_sp) {
    ...
    turn_result->best.rank_idx = 0;
    turn_result->best.num_plies = num_plies;
    for (int ply_idx = 0; ply_idx < num_plies; ply_idx++) {
      turn_result->best.ply_score[ply_idx] =
          stat_get_mean(simmed_play_get_score_stat(best_sp, ply_idx));
      turn_result->best.ply_bingo_pct[ply_idx] =
          stat_get_mean(simmed_play_get_bingo_stat(best_sp, ply_idx)) * 100.0;
    }
    populate_best_display_move(turn_result, game_get_board(ctx->game), ld);
  }
  turn_result->was_endgame = false;
...
static void analyze_with_endgame(const GameEvent *event,
  ...
  const bool actual_is_best =
      compare_moves_without_equity(&best_move, &actual_move_or_pass_if_phony,
                                   true) == -1;
