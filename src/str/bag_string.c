...
void string_builder_add_bag(const Bag *bag, const LetterDistribution *ld,
  ...
  for (int i = 1; i < ld_size; i++) {
    for (int j = 0; j < rack_get_letter(bag_as_rack, i); j++) {
      string_builder_add_user_visible_letter(ld, bag_string_builder, i);
    }
  }

  // Print the blanks at the end
  for (int i = 0; i < rack_get_letter(bag_as_rack, BLANK_MACHINE_LETTER); i++) {
    string_builder_add_user_visible_letter(ld, bag_string_builder,
