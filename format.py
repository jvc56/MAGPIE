...
def format_c_or_h_file(filepath: str, write_changes: bool) -> Tuple[bool, bool]:
    ...
    4. Applies clang-format-20 to the result of the above steps.
    5. If 'write_changes' is True, writes the final formatted content back to the file in place.
       Otherwise, it only compares the old and new content (after clang-format) and reports differences.
    ...
    # The content for clang-format will be the original content.
    ...
    # Now, apply clang-format-20 to the content generated by my script (or original if no includes)
    final_formatted_content_str = "".join(my_formatted_content) # Start with my script's output

    # Create a temporary file to apply clang-format-20
    # Use delete=False to ensure the file exists after creation for subprocess, then delete manually
    temp_file = None
    try:
        # Use the original file's extension for the temporary file to help clang-format
        with tempfile.NamedTemporaryFile(mode='w+', delete=False, encoding='utf-8', suffix=os.path.splitext(filepath)[1]) as temp_f:
            ...
            temp_f.flush() # Ensure content is written to disk before clang-format reads it

        # Apply clang-format-20 to the temporary file
        try:
            # Use check=True to raise CalledProcessError on non-zero exit codes
            # capture_output=True to prevent clang-format from printing to stdout/stderr directly
            subprocess.run(['clang-format-20', '-i', temp_file], check=True, capture_output=True)
        except FileNotFoundError:
            print(f"Warning: 'clang-format-20' not found. Skipping external formatting for '{filepath}'.")
        except subprocess.CalledProcessError as e:
            print(f"Error applying clang-format-20 to temporary file for '{filepath}': {e.stderr.decode().strip()}")
            # If clang-format-20 fails, we still proceed with my script's output for comparison
