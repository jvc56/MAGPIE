        ...
        if (game_state.board_entry_active) {
          ...
          if (key >= 0x20 && key < 0x7f) {
            ...
            if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')) {
              pthread_mutex_lock(&game_state.mutex);
              tui_move_entry_append_letter(&game_state, ch,
        ...
        if (key >= 0x20 && key < 0x7f) {
          ...
          if (*plen < per_field_cap && *plen + 1 < (int)buf_cap) {
            ...
            if (field_move && is_letter && typing_word) {
              ...
              int land_row = -1;
              int land_col = -1;
              if (!tui_move_entry_landing_square(&game_state, &land_row,
                                                 &land_col)) {
                land_row = -1;
                land_col = -1;
              }
              char glyph = '\0';
              if (tui_move_entry_resolve_letter(
                      &game_state, ch, ncinput_shift_p(&input), land_row,
                      land_col, &glyph) == TUI_TYPED_LETTER_REJECT) {
                pthread_mutex_unlock(&game_state.mutex);
                continue;
              }
              ch = glyph;
